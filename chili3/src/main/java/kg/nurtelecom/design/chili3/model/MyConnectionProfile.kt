package kg.nurtelecom.design.chili3.model

import android.content.res.Resources
import android.text.Spanned
import androidx.core.text.parseAsHtml
import kg.nurtelecom.design.chili2.util.PackageLeftOverUtils
import kg.nurtelecom.design.chili2.R as Chili2R
    ...
    fun getFormattedRemain(resources: Resources): CharSequence {
        return when (packageType) {
            ...
            PackageType.CALL -> {
                ...
                else getRemainingMinutesWithFormat(resources.getString(kg.nurtelecom.design.chili2.R.string.chili_minutes))
            }
    ...
    fun getFormattedLimit(resources: Resources): CharSequence {
        return when (packageType) {
            PackageType.INTERNET -> resources.getString(
                kg.nurtelecom.design.chili2.R.string.chili_leftover_from_limit,
                PackageLeftOverUtils.readableFileSize(limit, resources)
            )

            PackageType.CALL -> resources.getString(
                kg.nurtelecom.design.chili2.R.string.chili_leftover_from_limit,
                getLimitMinutesWithFormat(resources.getString(kg.nurtelecom.design.chili2.R.string.chili_minutes))
            )
